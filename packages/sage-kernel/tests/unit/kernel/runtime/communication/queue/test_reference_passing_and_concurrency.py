...
def worker_producer(queue_desc: BaseQueueDescriptor, worker_id: int, num_items: int, item_prefix: str = "item"):
    """生产者工作线程"""
    try:
...
def worker_consumer(queue_desc: BaseQueueDescriptor, worker_id: int, expected_items: int, timeout: float = 10.0):
    """消费者工作线程"""
    try:
        consumed_items = []
...
try:
    ...
    class QueueConsumerActor:
        ...
        def consume_items(self, queue_desc_dict: Dict[str, Any], actor_id: int, expected_items: int):
            """从队列消费物品"""
            try:
                # 从字典重建队列描述符
                from sage.kernel.runtime.communication.queue_descriptor import resolve_descriptor
...
class TestReferencePassingAndConcurrency:
    ...
    def test_python_queue_mixed_operations(self):
        ...
        with ThreadPoolExecutor(max_workers=num_workers) as executor:
            ...
            for i in range(num_workers):
                future = executor.submit(worker_mixed_operations, queue_desc, i, operations_per_worker)
    ...
    def test_ray_queue_actor_communication(self):
        ...
        try:
            ...
            print(f"配置: {num_producer_actors}个生产者Actor, {num_consumer_actors}个消费者Actor")
    ...
    def test_concurrent_stress_test(self):
        ...
        with ThreadPoolExecutor(max_workers=num_threads) as executor:
            ...
            for i in range(num_threads):
                future = executor.submit(worker_mixed_operations, queue_desc, i, operations_per_thread)
                futures.append(future)
