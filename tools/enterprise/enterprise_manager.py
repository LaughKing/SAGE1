...
class SAGEEnterpriseInstaller:
    ...
    def _find_project_root(self) -> Optional[Path]:
        ...
        for root in possible_roots:
            if root and root.exists():
                if (root / "tools" / "license").exists() and (root / "packages").exists():
                    return root
    ...
    def check_license_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            commercial_enabled = has_valid and any(f in features for f in enterprise_features)
    ...
    def validate_enterprise_installation(self) -> Dict[str, Any]:
        ...
        validation_tests = [
            ...
            {
                "name": "sage.apps.enterprise",
                "test": "from sage.apps.enterprise import vectordb",
                "description": "Enterprise application components"
            }
        ]
...
def main():
    ...
    # Check status
    check_parser = subparsers.add_parser("check", help="Check license and installation status")
    
    # Install enterprise
    install_parser = subparsers.add_parser("install", help="Install enterprise features")
    install_parser.add_argument("--license-key", help="License key to use")
    
    # Validate installation
    validate_parser = subparsers.add_parser("validate", help="Validate enterprise installation")
    
    # Show installation commands
    commands_parser = subparsers.add_parser("commands", help="Show installation commands")
    commands_parser.add_argument("--mode", choices=["standard", "enterprise", "dev", "individual"],
                               default="enterprise", help="Installation mode")
